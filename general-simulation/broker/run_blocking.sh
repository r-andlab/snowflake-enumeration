...
run_case() {
  ...
  for kv in "${extra_env[@]}"; do
    case "${kv}" in
      SNOWFLAKE_SIM_ATTACKER_CLIENT_PCT=*)
        attacker_pct="${kv#*=}"
        ;;
      SNOWFLAKE_SIM_ATTACKER_COUNT=*)
        fixed_attacker_count="${kv#*=}"
        ;;
  ...
  if [[ -n "${fixed_attacker_count}" ]]; then
    if ! [[ "${fixed_attacker_count}" =~ ^[0-9]+$ ]]; then
      echo "run_case: invalid SNOWFLAKE_SIM_ATTACKER_COUNT=${fixed_attacker_count} (need non-negative integer)" >&2
      exit 1
    fi
    effective_attacker_count="${fixed_attacker_count}"
    if ((effective_attacker_count >= effective_client_target)); then
      effective_attacker_count=$((effective_client_target - 1))
    fi
    if ((effective_attacker_count < 0)); then
      effective_attacker_count=0
    fi
    effective_client_target=$((effective_client_target - effective_attacker_count))
  elif [[ -n "${attacker_pct}" ]]; then
...
# DEFAULT (blocking attack).
run_case "default_blocking" 1

# Churn variants (blocking).
# SNOWFLAKE_SIM_CHURN_RATE_PCT — hourly proxy churn as % of fleet (replaced proxies per churn cycle).
run_case "churn_x2" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=5.4
run_case "churn_half" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=1.3
run_case "churn_x4" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=10.8
run_case "churn_x50" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=135
run_case "churn_quarter" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=0.675
run_case "churn_x10" 1 SNOWFLAKE_SIM_CHURN_RATE_PCT=27
...
for churn in 0.675 1.3 5.4 10.8 27; do
  cstr="${churn//./_}"
  run_case "churn_${cstr}_attackers_0_001" 1 \
    SNOWFLAKE_SIM_CHURN_RATE_PCT="${churn}" \
    SNOWFLAKE_SIM_ATTACKER_CLIENT_PCT=0.001
done

# Client target variants (blocking). SNOWFLAKE_SIM_CLIENT_TARGET_COUNT — half / quarter of CLIENT_BASELINE (default 61600).
# Fixed 32 attackers (not scaled from DEFAULT_ATTACKER_CLIENT_PCT) so client sweeps stay comparable.
run_case "clients_half" 1 \
  SNOWFLAKE_SIM_CLIENT_TARGET_COUNT=$((CLIENT_BASELINE / 2)) \
  SNOWFLAKE_SIM_ATTACKER_COUNT=32
run_case "clients_quarter" 1 \
  SNOWFLAKE_SIM_CLIENT_TARGET_COUNT=$((CLIENT_BASELINE / 4)) \
  SNOWFLAKE_SIM_ATTACKER_COUNT=32
run_case "clients_double" 1 \
  SNOWFLAKE_SIM_CLIENT_TARGET_COUNT=$((CLIENT_BASELINE * 2)) \
  SNOWFLAKE_SIM_ATTACKER_COUNT=32
run_case "clients_x4" 1 \
  SNOWFLAKE_SIM_CLIENT_TARGET_COUNT=$((CLIENT_BASELINE * 4)) \
  SNOWFLAKE_SIM_ATTACKER_COUNT=32
run_case "clients_x10" 1 \
  SNOWFLAKE_SIM_CLIENT_TARGET_COUNT=$((CLIENT_BASELINE * 10)) \
  SNOWFLAKE_SIM_ATTACKER_COUNT=32

# Connection duration variants (blocking). Optional sweeps; defaults above stay 3h mean / 30m stddev (10800 / 1800).
# 1.5 hours ± 15 minutes (mean 5400s, stddev 900s).
run_case "connection_1_5h_pm15m" 1 \
  SNOWFLAKE_SIM_CONNECTION_MEAN_SEC=5400 \
  SNOWFLAKE_SIM_CONNECTION_STDDEV_SEC=900
# 30 minutes ± 5 minutes (mean 1800s, stddev 300s).
run_case "connection_30m_pm5m" 1 \
  SNOWFLAKE_SIM_CONNECTION_MEAN_SEC=1800 \
  SNOWFLAKE_SIM_CONNECTION_STDDEV_SEC=300
run_case "connection_6h_pm1h" 1 \
  SNOWFLAKE_SIM_CONNECTION_MEAN_SEC=21600 \
  SNOWFLAKE_SIM_CONNECTION_STDDEV_SEC=3600
run_case "connection_9h_pm1h30m" 1 \
  SNOWFLAKE_SIM_CONNECTION_MEAN_SEC=32400 \
  SNOWFLAKE_SIM_CONNECTION_STDDEV_SEC=5400


# Defense sweep: all proxy types at 1s, 10s, 20s, 100s (blocking).
# SNOWFLAKE_SIM_*_POLL_SEC — seconds between polls for standalone / webext / iptproxy proxies.
for p in 10 120 240; do
  run_case "defense_equal_${p}s" 1 \
    SNOWFLAKE_SIM_STANDALONE_POLL_SEC="${p}" \
    SNOWFLAKE_SIM_WEBEXT_POLL_SEC="${p}" \
    SNOWFLAKE_SIM_IPTPROXY_POLL_SEC="${p}"
done

# Number-of-attackers sweep (blocking).
...
for pct in 0.001 0.01 0.05 0.1 0.5 1 5; do
  label="${pct//./_}"
  run_case "attackers_pct_${label}" 1 SNOWFLAKE_SIM_ATTACKER_CLIENT_PCT="${pct}"
done

# NAT distribution variants (blocking).

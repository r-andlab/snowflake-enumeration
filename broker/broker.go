...
type BrokerContext struct {
	...
	// Per-fake-minute snowflake removal stats (timeout); printed every fake minute.
	removalStatsMu     sync.Mutex
	lastPrintedMinute  time.Time
	removalsThisMinute int
}
...
// recordSnowflakeRemovalAndMaybeLog records one timeout removal and, when the next log bucket is reached, logs the number of removals for the completed bucket. In simulation the bucket is SNOWFLAKE_SIM_DEBUG_EVERY_SEC; otherwise 1 minute.
func (ctx *BrokerContext) recordSnowflakeRemovalAndMaybeLog() {
	now := sim.FakeTimeNow()
	interval := sim.RemovalLogInterval()
	currentBucket := now.Truncate(interval)
	ctx.removalStatsMu.Lock()
	defer ctx.removalStatsMu.Unlock()
	ctx.removalsThisMinute++
	if !ctx.lastPrintedMinute.IsZero() && currentBucket.After(ctx.lastPrintedMinute) {
		count := ctx.removalsThisMinute - 1
		log.Printf("snowflake removals: minute=%s count=%d", ctx.lastPrintedMinute.Format("2006-01-02 15:04"), count)
		ctx.removalsThisMinute = 1
		ctx.lastPrintedMinute = currentBucket
	} else if ctx.lastPrintedMinute.IsZero() {
		ctx.lastPrintedMinute = currentBucket
	}
}
...
func (ctx *BrokerContext) Broker() {
	for request := range ctx.proxyPolls {
		...
		// log.Printf("snowflakes length: %d, restricted snowflakes length: %d", ctx.snowflakes.Len(), ctx.restrictedSnowflakes.Len())
		...
		go func(request *ProxyPoll) {
			...
			case <-sim.FakeTimeAfter(proxyPollTimeout):
				// This snowflake is no longer available to serve clients.
				ctx.snowflakeLock.Lock()
				if snowflake.index != -1 {
					if request.natType == NATUnrestricted {
						// log.Printf("removing snowflake %s from unrestricted snowflakes", snowflake.id)
						heap.Remove(ctx.snowflakes, snowflake.index)
					} else {
						// log.Printf("removing snowflake %s from restricted snowflakes", snowflake.id)
					...
					ctx.recordSnowflakeRemovalAndMaybeLog()
				} else {
					ctx.snowflakeLock.Unlock()
				}
			}
...
func (ctx *BrokerContext) AddSnowflake(id string, proxyType string, natType string, clients int) *Snowflake {
	...
	// Buffered to avoid sender blocking races when proxy poll timeout fires near offer delivery.
...
func main() {
	...
	flag.BoolVar(&runSimulation, "simulate", false, "run proxy poll simulation instead of starting the broker server")
	...
	if ipCountPrefix != "" {
		...
		for _, name := range []string{"restricted", "unrestricted",
			...
			files[name], err = os.OpenFile(fmt.Sprintf("%s-%s-%s.log", ipCountPrefix,
				name, sim.FakeTimeNow().Format(time.RFC3339)), os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0644)
			if err != nil {
				log.Fatal(err.Error())
			}
		}
	...
	// GeoIP reload on SIGHUP disabled
